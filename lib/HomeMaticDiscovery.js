...
class HomeMaticDiscovery {
  ...
  _onClientMessage(message, remote) {
    ...
    // Reduce log verbosity on hot path; keep one informative line
    this.logger.log('info', `Discovery response from ${address}:${port}`);
    // this.logger.log('info', `Raw message: ${message.toString('hex')}`); // noisy – enable for debugging only
  ...
  async discover({ timeout = DISCOVER_TIMEOUT } = {}) {
    ...
    this.logger.log('info', 'Starting discovery...');

    let client;
    try {
      client = await this.getClient();
      await this.sendDiscoveryMessage(client);
      // Use Homey timer for lifecycle-safe timeout
      await new Promise(resolve => {
        this._timeoutHandle = this.homey.setTimeout(resolve, timeout);
      });
      this.logger.log('info', 'Discovery window elapsed');
    } catch (err) {
      ...
      this._clearTimeout();
      this._closeClient();
      this.logger.log('info', 'Discovery finished.');
    }
  }
  ...
  _clearTimeout() {
    if (this._timeoutHandle) {
      this.homey.clearTimeout(this._timeoutHandle);
      this._timeoutHandle = null;
    }
  }
